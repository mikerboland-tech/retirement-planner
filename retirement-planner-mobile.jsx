...
const FICA_SS_WAGE_BASE_2025 = 184500; // 2026 wage base (SSA); inflation-indexed below. Value is 2026.
...
const calculateFICA = (earnedIncome, filingStatus, yearsFromNow = 0, inflationRate = 0.03) => {
  ...
  // The Additional Medicare Tax threshold ($200k single / $250k MFJ) is statutory and NOT indexed.
...
// Standard Part B = $202.90/mo. partB = total monthly Part B premium for the tier; partD =
// monthly Part D IRMAA surcharge. Name kept for stability; values are 2026.
...
const MEDICARE_PART_B_STANDARD_2025 = 202.90; // 2026 standard Part B premium per month (CMS). Value is 2026.
...
const calculateHealthcareExpenses = (pi, myAge, spouseAge, yearsFromNow, primaryAlive, spouseAlive) => {
  ...
  people.forEach(person => {
    ...
    if (pi.ltcModel !== 'none' && (pi.healthcareModel === 'comprehensive' || pi.ltcModel === 'custom' || pi.ltcModel === 'default')) {
      ...
      // Bill only the fraction of this projection year that falls inside the final-ltcDuration
      // window (clamped to start no earlier than 65). Lifetime total = exactly ltcDuration months
      // (the old logic assigned 12 months/year plus a partial final year, over-billing ~40%).
...
const MEDICARE_PART_D_PREMIUM_2025 = 39;     // Avg monthly Part D base premium (2026)
...
// 2026 Long-term Capital Gains Rate Thresholds (IRS Rev. Proc. 2025-32). Name kept for stability.
  ...
  for (let year = currentYear; year <= currentYear + (planningAge - pi.myAge); year++) {
    ...
    accts.forEach(account => {
      ...
      if (ownerAlive && ownerAge >= account.startAge && ownerAge < account.stopAge) {
        ...
        // Employee contributions are deductible whether made by 'me' or 'spouse' (both credited).
        // 'both' lumps employee+employer and can't be split, so it is NOT deducted — split into
        // an employee row ('me'/'spouse') and an employer row for accurate tax modeling.
    ...
    if (isRetired) {
      ...
      // Lets the solver size the tax gross-up from the ACTUAL draw (Roth = tax-free,
      ...
      const estimateDrawComposition = (gross) => {
        let need = Math.max(0, gross - totalRMD);
      ...
      if (afterTaxGap > 0) {
        ...
        for (let i = 0; i < MAX_ITERATIONS_FOR_TAX_CALC; i++) { // Iterate to converge
          // Estimate the ACTUAL draw composition (replaces the old fixed 100%/70% guess that
          // grossed up withdrawals for phantom tax on Roth-first / brokerage-first strategies).
          const draw = estimateDrawComposition(testWithdrawal);
          const totalPreTaxFromWithdrawals = totalRMD + draw.preTax; // ordinary: RMD + voluntary pre-tax
          ...
          // SS taxable amount uses combined income, which INCLUDES capital gains (Pub 915).
          ...
          // Federal ORDINARY base excludes gains (taxed at LTCG rates below).
          ...
          // Retirement income for state exemption: pension only. State taxes gains as ordinary income.
    ...
    // "combined income" formula for SS taxability (Pub 915). They are removed from the
    // *federal ordinary* tax base below and taxed separately at preferential 0/15/20% rates.
    // State tax keeps the gains-inclusive figure (most states tax gains as ordinary income).
    ...
    // Federal ordinary tax on a gains-EXCLUDED base (gains taxed separately below). Previously
    // gains were left in this base AND taxed again via calculateCapitalGainsTax (double tax).
    ...
    // LTCG stacking position uses ordinary taxable income with gains EXCLUDED, after deduction.
    const baseDeduction = STANDARD_DEDUCTION_2026[effectiveFilingStatus] || STANDARD_DEDUCTION_2026.married_joint;
    const adjustedDeduction = baseDeduction * inflationFactor;
    ...
    // MAGI already includes capital gains via finalTotalTaxableIncome; add back only the
    // pre-tax deduction. (Previously gains were added a second time, overstating IRMAA/NIIT.)
    ...
    if (excessRMD > 0) {
      // Marginal tax on the excess RMD (ordinary income), by differencing taxes with vs. without
      // it. Federal uses a gains-EXCLUDED base (consistent with the federal ordinary tax above);
      // state uses the gains-INCLUSIVE base. Removing the excess also lowers taxable SS (Pub 915).
