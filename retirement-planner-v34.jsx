...
// MILESTONES
// ============================================
// A milestone is a meaningful point on the retirement timeline. Three categories:
//
// 1. CALCULATED — formula-driven, computed from the projection (e.g., "Coast FIRE Age",
//    "Lean FI", "Full FI"). User just enables/disables them.
//
// 2. DATE — user-specified age or date. Optional financial impact (e.g., "Daughter's
//    wedding (age 62, $30k)", "Sabbatical (no income, age 55–56)").
//
// 3. GOAL — user defines a target metric/value/age (e.g., "Net worth $2M by age 60").
//    The app shows whether they're on track based on the projection.

const MILESTONE_FORMULAS = {
  coastFire: {
    label: 'Coast FIRE Age',
    description: 'Age when you can stop contributing and still hit retirement goal at retirement age',
    icon: '🏖️',
    color: '#10b981',
  },
  leanFI: {
    label: 'Lean FI',
    description: '4% rule covers minimum essential expenses (50% of desired retirement income)',
    icon: '🌱',
    color: '#22c55e',
  },
  fullFI: {
    label: 'Full FI',
    description: '4% rule covers your full desired retirement income',
    icon: '🎯',
    color: '#3b82f6',
  },
  fatFI: {
    label: 'Fat FI',
    description: '4% rule covers 1.5× your desired retirement income (extra cushion)',
    icon: '💎',
    color: '#a855f7',
  },
  mortgageFree: {
    label: 'Mortgage Free',
    description: 'All mortgages paid off (based on payoff ages set in Assets tab)',
    icon: '🏠',
    color: '#f59e0b',
  },
  netWorthTarget: {
    label: 'Net Worth Target',
    description: 'First year crossing your target net worth',
    icon: '💰',
    color: '#eab308',
    requiresTarget: true,  // user must specify targetValue
  },
  portfolioTarget: {
    label: 'Portfolio Target',
    description: 'First year your liquid portfolio crosses target value',
    icon: '📈',
    color: '#06b6d4',
    requiresTarget: true,
  },
...
const MILESTONE_TARGET_METRICS = {
  netWorth: { label: 'Net Worth', extractor: (p) => p.totalNetWorth },
  portfolio: { label: 'Portfolio Balance', extractor: (p) => p.totalPortfolio },
  retirementIncome: { label: 'Annual Retirement Income', extractor: (p) => p.totalGuaranteedIncome },
};

const DEFAULT_MILESTONES = [
  { id: 1, type: 'calculated', name: 'Coast FIRE', formula: 'coastFire', enabled: true, showOnCharts: true, icon: '🏖️', color: '#10b981' },
  { id: 2, type: 'calculated', name: 'Full FI', formula: 'fullFI', enabled: true, showOnCharts: true, icon: '🎯', color: '#3b82f6' },
  { id: 3, type: 'calculated', name: 'Mortgage Free', formula: 'mortgageFree', enabled: true, showOnCharts: true, icon: '🏠', color: '#f59e0b' },
];

// Compute the achieved age for a milestone given the projections array.
// Returns null if not yet achieved within the projection window.
const computeMilestone = (milestone, projections, personalInfo) => {
  if (!projections || projections.length === 0) return null;
  
  if (milestone.type === 'calculated') {
    return computeCalculatedMilestone(milestone, projections, personalInfo);
  }
  if (milestone.type === 'date') {
    // User specified age or date — just return it
    if (milestone.age !== undefined && milestone.age !== null) {
      const proj = projections.find(p => p.myAge === milestone.age);
      return {
        achievedAge: milestone.age,
        achievedYear: proj?.year,
        netWorthAt: proj?.netWorth,
        portfolioAt: proj?.totalPortfolio,
        status: 'scheduled',
      };
    }
    return null;
  ...
  if (milestone.type === 'goal') {
    return computeGoalMilestone(milestone, projections, personalInfo);
  }
  return null;
};

const computeCalculatedMilestone = (milestone, projections, personalInfo) => {
  const formula = milestone.formula;
  const desiredIncome = personalInfo.desiredRetirementIncome || 80000;
  
  if (formula === 'coastFire') {
    // The age at which existing portfolio (with no further contributions) grows to
    // the FI target by retirement age, using a 4% safe withdrawal rate.
    const fiTarget = desiredIncome * 25;  // 4% rule => 25× expenses
    const retirementAge = personalInfo.myRetirementAge;
    // Use 7% real return as the growth assumption (after inflation)
    const realGrowthRate = 0.05;
    
    for (const p of projections) {
      if (p.myAge >= retirementAge) break;
      const yearsToRetirement = retirementAge - p.myAge;
      const projectedAtRetirement = (p.totalPortfolio || 0) * Math.pow(1 + realGrowthRate, yearsToRetirement);
      if (projectedAtRetirement >= fiTarget) {
        return {
          achievedAge: p.myAge,
          achievedYear: p.year,
          portfolioAt: p.totalPortfolio,
          netWorthAt: p.totalNetWorth,
          targetValue: fiTarget,
          status: 'achieved',
        };
      }
    }
    return { status: 'not_achieved', targetValue: fiTarget };
  }
  
  if (formula === 'leanFI' || formula === 'fullFI' || formula === 'fatFI') {
    const multiplier = formula === 'leanFI' ? 12.5 : formula === 'fullFI' ? 25 : 37.5;
    const target = desiredIncome * multiplier;
    
    for (const p of projections) {
      if ((p.totalPortfolio || 0) >= target) {
        return {
          achievedAge: p.myAge,
          achievedYear: p.year,
          portfolioAt: p.totalPortfolio,
    ...
    return { status: 'not_achieved', targetValue: target };
  }
  
  if (formula === 'mortgageFree') {
    // First year totalAssetDebt drops to 0 (or below a small threshold)
    for (const p of projections) {
      if ((p.assetDebt || 0) <= 100) {
        return {
          achievedAge: p.myAge,
          achievedYear: p.year,
          netWorthAt: p.totalNetWorth,
          status: p.myAge === personalInfo.myAge ? 'already_achieved' : 'achieved',
        };
      }
    }
  ...
  if (formula === 'netWorthTarget') {
    const target = milestone.targetValue || 1000000;
    for (const p of projections) {
      if ((p.totalNetWorth || 0) >= target) {
        return {
          achievedAge: p.myAge,
          achievedYear: p.year,
          netWorthAt: p.totalNetWorth,
...
const computeGoalMilestone = (milestone, projections, personalInfo) => {
  const target = milestone.targetValue;
  const targetAge = milestone.targetAge;
  const metric = MILESTONE_TARGET_METRICS[milestone.targetMetric || 'netWorth'];
  if (!metric) return null;
  
  // Find first year crossing the target
  let achievedAge = null, achievedYear = null;
  for (const p of projections) {
    if ((metric.extractor(p) || 0) >= target) {
      achievedAge = p.myAge;
      achievedYear = p.year;
      break;
    }
  ...
  // Determine status relative to targetAge
  let status;
  if (achievedAge === null) {
    status = 'behind';  // never achieved
  } else if (achievedAge < targetAge) {
  ...
  // Find what value will be hit at the target age
  const atTargetAge = projections.find(p => p.myAge === targetAge);
  const valueAtTarget = atTargetAge ? metric.extractor(atTargetAge) : null;
  ...
  return {
    ...
    targetAge,
    valueAtTarget,
    metric: milestone.targetMetric,
    status,
  };
};

// ============================================
// CAREER PHASES — Multi-phase retirement
// ============================================
// A phase is a named time window with a status (working/semi-retired/retired/sabbatical).
// Phases are DESCRIPTIVE — they bundle income streams, contribution timings, and
// spending changes that users want to think about as one unit ("my consulting years"),
// but the authoritative data still lives in incomeStreams, accounts.contribution*, etc.
//
// This means phases compose perfectly with everything we've already built:
//   - Income streams already have startAge/endAge
//   - Accounts already have contribution startAge/stopAge
//   - Recurring expenses already have startAge/endAge
//   - One-time events already exist for big lump-sum moments
//
// A phase is a way to GROUP these related items, give them a label, and visualize
// them on the timeline. When a user creates a phase, it can offer to update the
// underlying items in one batch. When a user looks at the timeline, they see the
// phase bands rather than reasoning about 7 different income streams.

const PHASE_STATUSES = {
  working:       { label: 'Working',       color: '#3b82f6', icon: '💼', description: 'Full-time earned income, full contributions' },
  semi_retired:  { label: 'Semi-Retired',  color: '#a855f7', icon: '⚖️', description: 'Part-time income, possibly reduced contributions' },
  sabbatical:    { label: 'Sabbatical',    color: '#f59e0b', icon: '🌴', description: 'Career break — no earned income' },
  retired:       { label: 'Retired',       color: '#10b981', icon: '🏖️', description: 'No earned income; living off portfolio + pensions/SS' },
  bridge:        { label: 'Bridge Income', color: '#06b6d4', icon: '🌉', description: 'Reduced/transitional income before full retirement' },
};

const DEFAULT_PHASES = [];  // Empty by default — users create their own

// Helper: compute the effective income/savings rate for a year given the current data.
// Used to render phase bands and detect inconsistencies.
const summarizePhaseFromData = (phase, accounts, incomeStreams, recurringExpenses, personalInfo) => {
  const startAge = phase.startAge;
  const endAge = phase.endAge !== null && phase.endAge !== undefined ? phase.endAge : personalInfo.myLifeExpectancy;
  
  // Aggregate earned income across the phase window (approx — uses startAge value)
  let earnedIncomeAtStart = 0, otherIncomeAtStart = 0;
  incomeStreams.forEach(s => {
    if (startAge >= s.startAge && startAge <= s.endAge) {
      // Income at phase start, growing with COLA
      const yearsSince = startAge - s.startAge;
      const adjusted = s.amount * Math.pow(1 + (s.cola || 0), Math.max(0, yearsSince));
      if (s.type === 'earned_income') earnedIncomeAtStart += adjusted;
      else otherIncomeAtStart += adjusted;
    }
  });
  
  // Aggregate contributions
  let contributionsAtStart = 0;
  accounts.forEach(a => {
    const ownerAge = a.owner === 'spouse' ? (personalInfo.spouseAge + (startAge - personalInfo.myAge)) : startAge;
    if (ownerAge >= a.startAge && ownerAge < a.stopAge) {
      const yearsSince = ownerAge - a.startAge;
      contributionsAtStart += a.contribution * Math.pow(1 + (a.contributionGrowth || 0), Math.max(0, yearsSince));
    }
  });
  
  // Aggregate recurring expenses active in this phase
  let recurringAtStart = 0;
  recurringExpenses.forEach(e => {
    if (startAge >= e.startAge && startAge <= e.endAge) {
      recurringAtStart += e.amount;
    }
  });
  ...
  return {
    earnedIncomeAtStart,
    otherIncomeAtStart,
    contributionsAtStart,
    recurringAtStart,
    durationYears: endAge - startAge + 1,
  };
};

// Detect overlaps and gaps in a list of phases. Returns warnings.
const validatePhases = (phases, personalInfo) => {
  const warnings = [];
  if (phases.length === 0) return warnings;
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    ...
    const aEnd = a.endAge !== null && a.endAge !== undefined ? a.endAge : personalInfo.myLifeExpectancy;
    if (aEnd >= b.startAge) {
      warnings.push({
        type: 'overlap',
        severity: 'error',
        message: `"${a.name}" (ends age ${aEnd}) overlaps with "${b.name}" (starts age ${b.startAge}). Phases should not overlap.`,
      });
    }
  }
  ...
  for (let i = 0; i < sorted.length - 1; i++) {
    const a = sorted[i], b = sorted[i + 1];
    const aEnd = a.endAge !== null && a.endAge !== undefined ? a.endAge : null;
    if (aEnd !== null && aEnd + 1 < b.startAge) {
      warnings.push({
        type: 'gap',
        severity: 'warning',
        message: `Gap between "${a.name}" (ends age ${aEnd}) and "${b.name}" (starts age ${b.startAge}). Ages ${aEnd + 1}–${b.startAge - 1} are unaccounted.`,
      });
    }
  }
  
  // Check that first phase starts at current age or later
  if (sorted[0].startAge > personalInfo.myAge) {
    warnings.push({
      type: 'gap',
      severity: 'info',
      message: `First phase "${sorted[0].name}" starts at age ${sorted[0].startAge}, but you're currently ${personalInfo.myAge}. Ages ${personalInfo.myAge}–${sorted[0].startAge - 1} have no phase defined.`,
    });
  }
  
  // Check that last phase covers life expectancy
  const lastPhase = sorted[sorted.length - 1];
  const lastEnd = lastPhase.endAge !== null && lastPhase.endAge !== undefined ? lastPhase.endAge : personalInfo.myLifeExpectancy;
  if (lastEnd < personalInfo.myLifeExpectancy) {
    warnings.push({
      type: 'gap',
      severity: 'info',
      message: `Last phase "${lastPhase.name}" ends at age ${lastEnd}, but your life expectancy is ${personalInfo.myLifeExpectancy}. Ages ${lastEnd + 1}–${personalInfo.myLifeExpectancy} have no phase defined.`,
    });
  }
...
// ============================================
// CASH-FLOW PRIORITY ENGINE
// ============================================
// User defines an ordered list of priorities for surplus dollars (income
// after tax − essential expenses − minimum debt payments). Each rule has
// a category and a target amount; dollars flow through the list in order
// until surplus is exhausted.
//
// This is a UX HELPER, not a runtime engine. Users review the recommended
// contributions and can apply them to their actual account contribution
// settings via a one-click "apply" action. The projection engine continues
// to use the explicit per-account contribution values — this tool just
// helps users figure out what those values should be.

const PRIORITY_RULE_TYPES = {
  debt_min:           { label: 'Minimum debt payments',     icon: '💳', description: 'Required minimum on all debts (always first)', defaultAmount: 0, fixed: true },
  employer_match:     { label: '401(k) employer match',     icon: '🎁', description: 'Get the full company match — free money', defaultAmount: 6, isPercent: true },
  hsa:                { label: 'Max HSA',                   icon: '🏥', description: 'Triple tax advantage if eligible', defaultAmount: 4300 },
  high_int_debt:      { label: 'Pay off high-interest debt', icon: '🔥', description: 'Debts over 6% APR', defaultAmount: 0, debtThreshold: 0.06 },
  roth_ira:           { label: 'Max Roth IRA',              icon: '🌱', description: 'Tax-free growth, no RMDs', defaultAmount: 7000 },
  retirement_max:     { label: 'Max 401(k)',                icon: '🏛️', description: 'Pre-tax retirement savings', defaultAmount: 23500 },
  med_int_debt:       { label: 'Pay off medium-interest debt', icon: '⚠️', description: 'Debts 3-6% APR', defaultAmount: 0, debtThreshold: 0.03 },
  brokerage_savings:  { label: 'Brokerage / taxable',       icon: '📈', description: 'Flexible long-term savings', defaultAmount: 0, takesAll: true },
  low_int_debt:       { label: 'Pay off low-interest debt', icon: '🐢', description: 'Debts under 3% APR (often skip)', defaultAmount: 0 },
  emergency_fund:     { label: 'Emergency fund',            icon: '🛟', description: '3-6 months of essential expenses', defaultAmount: 10000 },
};

const DEFAULT_PRIORITY_RULES = [
  { id: 1, type: 'debt_min',          enabled: true, locked: true },   // Always first
  { id: 2, type: 'employer_match',    enabled: true, percentOfSalary: 6 },
  { id: 3, type: 'hsa',               enabled: true, amount: 4300 },
  { id: 4, type: 'high_int_debt',     enabled: true },
  { id: 5, type: 'roth_ira',          enabled: true, amount: 7000 },
  { id: 6, type: 'retirement_max',    enabled: true, amount: 23500 },
  { id: 7, type: 'med_int_debt',      enabled: true },
  { id: 8, type: 'brokerage_savings', enabled: true },
  { id: 9, type: 'low_int_debt',      enabled: false },  // Skip by default — keep cheap money working
];

// Compute current-year cash-flow allocation given priority rules and user's data.
// Returns an array of allocations matching the priority list, plus surplus/deficit info.
const computeCashFlowAllocation = (rules, accounts, incomeStreams, recurringExpenses, personalInfo, projections) => {
  // Find current year in projections (the first one matching myAge)
  const current = projections.find(p => p.myAge === personalInfo.myAge);
  if (!current) {
    return { allocations: [], totalIncome: 0, totalTax: 0, totalEssential: 0, surplus: 0, allocated: 0, unallocated: 0, error: 'No current-year projection available' };
  }
  
  // Total gross income this year
  const totalIncome = (current.totalSalary || 0) + (current.totalSocialSecurity || 0) + (current.totalPension || 0) + (current.totalOtherIncome || 0);
  // Total tax burden — use the engine's calculation
  const totalTax = (current.federalTax || 0) + (current.stateTax || 0) + (current.ficaTax || 0) + (current.niitTax || 0) + (current.irmaaTax || 0);
  // Essential expenses: recurring + healthcare + minimum debt payments (we approximate via desiredRetirementIncome ratio)
  // For working years, "essential" is a chunk of current income
  const isWorking = personalInfo.myAge < personalInfo.myRetirementAge;
  const essentialExpenses = isWorking
    ? (personalInfo.currentExpenses || 60000)
    : (personalInfo.desiredRetirementIncome || 80000);
  
  // Recurring expenses active now
  let recurringTotal = 0;
  recurringExpenses.forEach(e => {
    if (personalInfo.myAge >= e.startAge && personalInfo.myAge <= e.endAge) {
      recurringTotal += e.amount;
    }
  });
  
  // Surplus is income minus tax minus essentials minus recurring expenses
  const totalEssential = essentialExpenses + recurringTotal;
  let surplus = totalIncome - totalTax - totalEssential;
  let remaining = Math.max(0, surplus);
  
  // Get a primary salary for percent-based rules (employer match)
  const salaryStream = incomeStreams.find(s => s.type === 'earned_income' && s.owner === 'me' &&
    personalInfo.myAge >= s.startAge && personalInfo.myAge <= s.endAge);
  const salary = salaryStream ? salaryStream.amount * Math.pow(1 + (salaryStream.cola || 0), Math.max(0, personalInfo.myAge - salaryStream.startAge)) : 0;
  
  // Walk priority list; allocate surplus dollars to each enabled rule
  const allocations = [];
  rules.forEach(rule => {
    const def = PRIORITY_RULE_TYPES[rule.type];
    if (!def) return;
    if (!rule.enabled) {
      allocations.push({ ...rule, def, recommendedAmount: 0, allocated: 0, status: 'disabled', reason: 'Skipped (disabled)' });
      return;
    }
    
    let target;
    if (rule.type === 'employer_match') {
      target = salary * ((rule.percentOfSalary || 6) / 100);
    } else if (rule.type === 'brokerage_savings') {
      target = remaining;  // Takes whatever is left
    } else if (rule.amount !== undefined) {
      target = rule.amount;
    } else {
      target = def.defaultAmount || 0;
    }
    ...
    let reason = '';
    if (allocated < target && allocated > 0) {
      status = 'partial';
      reason = `Surplus ran out — only $${Math.round(allocated).toLocaleString()} of $${Math.round(target).toLocaleString()} target`;
    } else if (allocated === 0 && target > 0) {
      status = 'unfunded';
      reason = `Surplus exhausted before reaching this priority`;
    } else if (target === 0) {
      status = 'na';
      reason = 'No target amount';
    }
    
    allocations.push({ ...rule, def, recommendedAmount: target, allocated, status, reason, salary });
  });
  ...
  return {
    ...
    totalEssential,
    surplus,
    allocated: Math.max(0, surplus) - remaining,
...
// ============================================
function DashboardTab({ accounts, assets, computeProjections, dashboardVisibility, incomeStreams, milestones, oneTimeEvents, personalInfo, phases, projections, recurringExpenses, setDashboardVisibility, setShowDashboardSettings, showDashboardSettings }) {
  ...
  // Compute milestone reference lines for charts.
  // Only show milestones that are: enabled, marked showOnCharts, and have a calculated achievement age
  const milestoneMarkers = useMemo(() => {
    if (!milestones || milestones.length === 0) return [];
    return milestones
      .filter(m => m.enabled && m.showOnCharts)
      .map(m => {
        const result = computeMilestone(m, projections, personalInfo);
        if (!result || result.achievedAge === undefined || result.achievedAge === null) return null;
        return {
          age: result.achievedAge,
          label: m.name,
          icon: m.icon || '🎯',
      ...
      .filter(Boolean);
  }, [milestones, projections, personalInfo]);
  
  // Compute phase boundary markers for charts. Each phase START becomes a faint
  // vertical reference line. We don't draw bands because Recharts' default
  // background painting doesn't blend cleanly with stacked areas. The phase
  // boundaries paired with the legend below are clearer than colored bands.
  const phaseMarkers = useMemo(() => {
    if (!phases || phases.length === 0) return [];
    const sorted = [...phases].sort((a, b) => a.startAge - b.startAge);
    return sorted.map(p => {
      const status = PHASE_STATUSES[p.status] || PHASE_STATUSES.working;
      return {
        age: p.startAge,
        label: p.name,
        icon: status.icon,
            ...
            <ComposedChart data={netWorthData}>
              ...
              {milestoneMarkers.map((m, i) => (
                <ReferenceLine 
                  ...
                  strokeDasharray="3 3" 
                  strokeOpacity={0.7}
                  label={{ value: `${m.icon} ${m.label}`, position: 'top', fill: m.color, fontSize: 10 }}
                />
              ))}
              {phaseMarkers.map((p, i) => (
                <ReferenceLine
                  key={`phase-nw-${i}`}
                  x={p.age}
                  stroke={p.color}
                  strokeDasharray="2 4"
                  strokeOpacity={0.5}
                  label={{ value: `${p.icon} ${p.label}`, position: 'insideBottom', fill: p.color, fontSize: 9 }}
                />
              ))}
            </ComposedChart>
          </ResponsiveContainer>
...
// ============================================
// MilestonesTab — Goal tracking and life event milestones
// ============================================
function MilestonesTab({ milestones, setMilestones, projections, personalInfo }) {
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingMilestone, setEditingMilestone] = useState(null);
  const [filterType, setFilterType] = useState('all');
  
  // Compute results for each milestone
  const milestoneResults = milestones.map(m => ({
    milestone: m,
    result: computeMilestone(m, projections, personalInfo)
  }));
  
  // Summary stats
  const achieved = milestoneResults.filter(r => r.result?.status === 'achieved' || r.result?.status === 'already_achieved').length;
  const onTrackOrAhead = milestoneResults.filter(r => r.result?.status === 'on_track' || r.result?.status === 'ahead').length;
  const behind = milestoneResults.filter(r => r.result?.status === 'behind').length;
  const notAchieved = milestoneResults.filter(r => r.result?.status === 'not_achieved').length;
  
  const filteredResults = filterType === 'all' 
    ? milestoneResults 
    : milestoneResults.filter(r => r.milestone.type === filterType);
  
  const toggleEnabled = (id) => {
    setMilestones(prev => prev.map(m => m.id === id ? { ...m, enabled: !m.enabled } : m));
  };
  
  const toggleShowOnCharts = (id) => {
    setMilestones(prev => prev.map(m => m.id === id ? { ...m, showOnCharts: !m.showOnCharts } : m));
  };
  ...
  const saveMilestone = (data) => {
    if (editingMilestone) {
      setMilestones(prev => prev.map(m => m.id === editingMilestone.id ? { ...editingMilestone, ...data } : m));
    } else {
      const newId = Math.max(0, ...milestones.map(m => m.id)) + 1;
      setMilestones(prev => [...prev, { id: newId, enabled: true, showOnCharts: true, ...data }]);
    }
  ...
  const statusBadge = (status) => {
    const styles = {
      achieved:        { label: '✓ Achieved', cls: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' },
      already_achieved:{ label: '✓ Already there', cls: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' },
      ahead:           { label: '⚡ Ahead of schedule', cls: 'bg-blue-500/20 text-blue-300 border-blue-500/40' },
      on_track:        { label: '✓ On track', cls: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40' },
      behind:          { label: '⚠ Behind schedule', cls: 'bg-amber-500/20 text-amber-300 border-amber-500/40' },
      scheduled:       { label: '📅 Scheduled', cls: 'bg-slate-500/20 text-slate-300 border-slate-500/40' },
      not_achieved:    { label: '✗ Not achieved in plan window', cls: 'bg-red-500/20 text-red-300 border-red-500/40' },
    };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold text-slate-100 mb-2">Milestones</h3>
        <p className="text-slate-400 text-sm">
          Track meaningful points on your retirement journey: financial independence dates, life events,
          and personal goals. Calculated milestones (Coast FIRE, Full FI, Mortgage Free) are computed
          automatically from your projections. Custom milestones let you mark specific dates or goals.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className={cardStyle}>
          <div className="text-xs text-slate-500 mb-1">On Track / Ahead</div>
          <div className="text-3xl font-bold text-blue-400">{onTrackOrAhead}</div>
        </div>
        <div className={cardStyle}>
          <div className="text-xs text-slate-500 mb-1">Behind Schedule</div>
          <div className="text-3xl font-bold text-amber-400">{behind}</div>
        </div>
        <div className={cardStyle}>
          <div className="text-xs text-slate-500 mb-1">Not Yet Reachable</div>
          <div className="text-3xl font-bold text-red-400">{notAchieved}</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex gap-1">
          {['all', 'calculated', 'date', 'goal'].map(t => (
            <button
              key={t}
        ...
        <div className="flex-1" />
        <button
          onClick={() => { setEditingMilestone(null); setShowAddModal(true); }}
      ...
      <div className="space-y-3">
        {filteredResults.length === 0 ? (
          <div className={`${cardStyle} text-center text-slate-400 py-8`}>
            No milestones in this category. Click "Add Milestone" to create one.
          </div>
        ) : filteredResults.map(({ milestone, result }) => (
          <div key={milestone.id} className={`${cardStyle} ${!milestone.enabled ? 'opacity-50' : ''}`}>
            <div className="flex items-start gap-4">
              <div className="text-3xl flex-shrink-0">{milestone.icon || '🎯'}</div>
              
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <h4 className="text-lg font-semibold text-slate-100">{milestone.name}</h4>
                ...
                {result && (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 text-sm">
                    {result.achievedAge !== undefined && result.achievedAge !== null && (
                      <div>
                        <div className="text-xs text-slate-500">Achieved at age</div>
                        <div className="text-lg font-bold text-emerald-400">{result.achievedAge}</div>
                        {result.achievedYear && (
                          <div className="text-xs text-slate-500">{result.achievedYear}</div>
                        )}
                      </div>
                    )}
                    {result.targetValue !== undefined && (
                      <div>
                        ...
                        <div className="text-lg font-bold text-slate-200">{formatCurrency(result.targetValue)}</div>
                      </div>
                    )}
                    {result.targetAge !== undefined && (
                      <div>
                        <div className="text-xs text-slate-500">Target age</div>
                        <div className="text-lg font-bold text-slate-200">{result.targetAge}</div>
                      </div>
                    )}
                    {result.netWorthAt !== undefined && result.netWorthAt !== null && (
                      <div>
                        <div className="text-xs text-slate-500">Net worth at milestone</div>
                        <div className="text-lg font-bold text-amber-300">{formatCurrency(result.netWorthAt)}</div>
                      </div>
                    )}
                    {result.portfolioAt !== undefined && result.portfolioAt !== null && (
                      <div>
                        <div className="text-xs text-slate-500">Portfolio at milestone</div>
                        <div className="text-lg font-bold text-blue-300">{formatCurrency(result.portfolioAt)}</div>
                      </div>
                    )}
                    {result.valueAtTarget !== undefined && result.valueAtTarget !== null && (
                      <div>
                        <div className="text-xs text-slate-500">Projected at target age</div>
                        <div className="text-lg font-bold text-amber-300">{formatCurrency(result.valueAtTarget)}</div>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="flex flex-col gap-1.5 flex-shrink-0">
                <button
                  onClick={() => toggleEnabled(milestone.id)}
                  className={`text-xs px-2 py-1 rounded border transition-all ${
                    milestone.enabled
                      ? 'bg-emerald-500/10 text-emerald-300 border-emerald-500/30 hover:bg-emerald-500/20'
                      : 'bg-slate-700 text-slate-400 border-slate-600 hover:bg-slate-600'
                  }`}
                  title="Toggle whether this milestone is active"
                >
                  {milestone.enabled ? '✓ Enabled' : '○ Disabled'}
                </button>
                <button
                  onClick={() => toggleShowOnCharts(milestone.id)}
                  className={`text-xs px-2 py-1 rounded border transition-all ${
                    milestone.showOnCharts
                      ? 'bg-blue-500/10 text-blue-300 border-blue-500/30 hover:bg-blue-500/20'
                      : 'bg-slate-700 text-slate-400 border-slate-600 hover:bg-slate-600'
                  }`}
                  title="Toggle whether this milestone appears as a marker on dashboard charts"
                >
                  {milestone.showOnCharts ? '📊 Charts on' : '📊 Charts off'}
                </button>
                {milestone.type !== 'calculated' && (
                  <button
                    onClick={() => { setEditingMilestone(milestone); setShowAddModal(true); }}
                    className="text-xs px-2 py-1 rounded border bg-slate-700 text-slate-300 border-slate-600 hover:bg-slate-600 transition-all"
                  >
                    ✏️ Edit
                  </button>
                )}
                <button
                  onClick={() => {
                    if (window.confirm(`Delete milestone "${milestone.name}"?`)) deleteMilestone(milestone.id);
                  }}
                  className="text-xs px-2 py-1 rounded border bg-red-500/10 text-red-300 border-red-500/30 hover:bg-red-500/20 transition-all"
                >
                  🗑️ Delete
                </button>
              </div>
      ...
      {/* Quick-add calculated milestones not already added */}
      <div className={cardStyle}>
        <h4 className="text-md font-semibold text-slate-200 mb-3">Add Calculated Milestone</h4>
        <p className="text-xs text-slate-400 mb-3">Click any to add — these compute automatically from your projection data:</p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {Object.entries(MILESTONE_FORMULAS).map(([key, def]) => {
            const alreadyAdded = milestones.some(m => m.type === 'calculated' && m.formula === key);
            return (
              <button
                key={key}
                disabled={alreadyAdded && !def.requiresTarget}
                onClick={() => {
                  const newId = Math.max(0, ...milestones.map(m => m.id)) + 1;
                  setMilestones(prev => [...prev, {
                    ...
                    enabled: true,
                    showOnCharts: true,
                    targetValue: def.requiresTarget ? 1000000 : undefined,
                  }]);
                }}
                className={`flex items-start gap-2 p-3 rounded-lg border text-left transition-all ${
                  alreadyAdded && !def.requiresTarget
                    ? 'opacity-30 cursor-not-allowed bg-slate-800 border-slate-700'
                    : 'bg-slate-800 hover:bg-slate-700 border-slate-700 hover:border-amber-500/50'
                }`}
              >
                ...
                <div className="min-w-0">
                  ...
                  {alreadyAdded && !def.requiresTarget && <div className="text-xs text-amber-300 mt-1">Already added</div>}
                </div>
              </button>
            );
          })}
      ...
      {showAddModal && (
        <MilestoneModal
          ...
          onCancel={() => { setShowAddModal(false); setEditingMilestone(null); }}
        />
      )}
    </div>
  );
...
// Modal for creating or editing date/goal milestones
function MilestoneModal({ milestone, onSave, onCancel }) {
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {!milestone && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  onClick={() => setFormData({...formData, type: 'date'})}
                  className={`p-3 rounded-lg border text-left transition-all ${
                    formData.type === 'date' 
                      ? 'border-amber-500 bg-amber-500/10' 
                      : 'border-slate-700 bg-slate-900 hover:border-slate-600'
                ...
                >
                  ...
                  <div className="text-xs text-slate-400">Specific age (e.g., kids' wedding)</div>
                </button>
                <button
                ...
                >
                  <div className="text-sm font-medium text-slate-200">🎯 Goal</div>
                  <div className="text-xs text-slate-400">Target value by target age</div>
                </button>
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <input 
              ...
              placeholder={formData.type === 'date' ? 'e.g., Daughter\u2019s wedding' : 'e.g., $2M net worth'}
            />
          </div>
          ...
          {formData.type === 'date' && (
            <div>
              <label className={labelStyle}>Age at milestone</label>
              <input 
                type="number" 
          ...
          {formData.type === 'goal' && (
            <>
              <div>
                <label className={labelStyle}>Target metric</label>
                <select 
                  value={formData.targetMetric || 'netWorth'} 
                  onChange={e => setFormData({...formData, targetMetric: e.target.value})} 
                  className={inputStyle}
                >
                  {Object.entries(MILESTONE_TARGET_METRICS).map(([key, def]) => (
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={labelStyle}>Target value ($)</label>
                  <input 
                    type="number" 
                    value={formData.targetValue || 1000000} 
                ...
                <div>
                  <label className={labelStyle}>Target age</label>
                  <input 
                    type="number" 
                    value={formData.targetAge || 65} 
          ...
          {formData.type === 'calculated' && formData.formula === 'netWorthTarget' || formData.type === 'calculated' && formData.formula === 'portfolioTarget' ? (
            <div>
              <label className={labelStyle}>Target value ($)</label>
              <input 
                type="number" 
        ...
        <div className="flex gap-2 mt-6">
          <button onClick={onCancel} className={`${buttonSecondary} flex-1`}>Cancel</button>
          <button onClick={handleSave} className={`${buttonPrimary} flex-1`}>
...
// ============================================
// PhasesTab — Multi-phase retirement
// ============================================
function PhasesTab({ phases, setPhases, accounts, incomeStreams, recurringExpenses, personalInfo }) {
  const [showAddModal, setShowAddModal] = useState(false);
  const [editingPhase, setEditingPhase] = useState(null);
  
  const sortedPhases = [...phases].sort((a, b) => a.startAge - b.startAge);
  const warnings = validatePhases(phases, personalInfo);
  ...
  const savePhase = (data) => {
    if (editingPhase) {
      setPhases(prev => prev.map(p => p.id === editingPhase.id ? { ...editingPhase, ...data } : p));
    } else {
      const newId = Math.max(0, ...phases.map(p => p.id)) + 1;
      setPhases(prev => [...prev, { id: newId, ...data }]);
    }
    setShowAddModal(false);
    setEditingPhase(null);
  };
  
  // Generate phase suggestions for first-time users
  const suggestStandardPhases = () => {
    const myAge = personalInfo.myAge;
    const retAge = personalInfo.myRetirementAge;
    const lifeExp = personalInfo.myLifeExpectancy;
    ...
    if (myAge < retAge) {
      suggestions.push({
        ...
        startAge: myAge,
        endAge: retAge - 1,
        notes: 'Full-time employment with retirement contributions',
      });
    }
    suggestions.push({
      id: 2,
      ...
      startAge: retAge,
      endAge: lifeExp,
      notes: 'Living off portfolio + Social Security + pension',
    });
    setPhases(suggestions);
  };
  
  // Compute timeline range
  const timelineStart = personalInfo.myAge;
  const timelineEnd = personalInfo.myLifeExpectancy;
  const timelineSpan = timelineEnd - timelineStart;
  
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold text-slate-100 mb-2">Career Phases</h3>
        <p className="text-slate-400 text-sm">
          Define the major chapters of your financial life: working years, semi-retirement, sabbaticals, full retirement.
          Phases visualize the timeline of your existing income streams, contributions, and expenses — they don't replace those settings, just group them so you can think about your plan as a sequence of life chapters.
        </p>
      </div>
      
      {phases.length === 0 ? (
        // Empty state — offer to create defaults
        <div className={`${cardStyle} text-center py-8`}>
          <div className="text-5xl mb-4">📅</div>
          <h4 className="text-lg font-semibold text-slate-100 mb-2">No phases defined yet</h4>
          <p className="text-sm text-slate-400 mb-6 max-w-md mx-auto">
            Phases are an optional but powerful way to think about your plan in chapters. The simplest setup is two phases: "Working Years" and "Retirement". You can also add semi-retirement, sabbaticals, or bridge-income years.
          </p>
          <div className="flex flex-wrap gap-2 justify-center">
            <button onClick={suggestStandardPhases} className={buttonPrimary}>
              ✨ Create Standard Phases
            </button>
            <button onClick={() => { setEditingPhase(null); setShowAddModal(true); }} className={buttonSecondary}>
              + Add Custom Phase
            </button>
          </div>
        </div>
        ...
        <>
          ...
          <div className={cardStyle}>
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-lg font-semibold text-slate-100">Timeline</h4>
              <span className="text-xs text-slate-500">Age {timelineStart} → {timelineEnd}</span>
            </div>
            <div className="relative h-16 bg-slate-900/60 rounded-lg overflow-hidden border border-slate-700">
              {sortedPhases.map(phase => {
                const startAge = phase.startAge;
                const endAge = phase.endAge !== null && phase.endAge !== undefined ? phase.endAge : timelineEnd;
                const left = ((startAge - timelineStart) / timelineSpan) * 100;
                const width = ((endAge - startAge + 1) / timelineSpan) * 100;
                const status = PHASE_STATUSES[phase.status] || PHASE_STATUSES.working;
                return (
                  <div
                    key={phase.id}
                    className="absolute top-0 bottom-0 flex items-center justify-center text-xs font-medium text-slate-100 border-r border-slate-900/50 cursor-pointer hover:opacity-90 transition-opacity"
                    style={{
                      left: `${left}%`,
                      width: `${width}%`,
                      backgroundColor: status.color + 'cc',  // 80% opacity
                    }}
                    onClick={() => { setEditingPhase(phase); setShowAddModal(true); }}
                    title={`${phase.name}: age ${startAge}–${endAge}`}
                  >
              ...
              <div className="absolute top-0 bottom-0 left-0 right-0 pointer-events-none">
                {[
                  personalInfo.myRetirementAge,
                  62, 65, 67, 70, 75
                ].filter(age => age >= timelineStart && age <= timelineEnd).map(age => {
                  const left = ((age - timelineStart) / timelineSpan) * 100;
                  return (
                    <div
                      key={age}
                      className="absolute top-0 bottom-0 w-px bg-slate-100/30"
                      style={{ left: `${left}%` }}
                    >
                      <div className="absolute -top-1 -translate-x-1/2 text-[10px] text-slate-400 whitespace-nowrap">
                        {age}
                      </div>
                    </div>
                  );
            ...
            <div className="mt-3 flex flex-wrap gap-3 text-xs">
              {Object.entries(PHASE_STATUSES).map(([key, def]) => (
                <div key={key} className="flex items-center gap-1.5">
                  <div className="w-3 h-3 rounded-sm" style={{ backgroundColor: def.color }} />
                  <span className="text-slate-400">{def.icon} {def.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {warnings.length > 0 && (
            <div className="space-y-2">
              {warnings.map((w, i) => {
                const styleMap = {
                  error: 'bg-red-900/20 border-red-500/40 text-red-300',
                  warning: 'bg-amber-900/20 border-amber-500/40 text-amber-300',
                  info: 'bg-slate-800/40 border-slate-500/40 text-slate-300',
                };
                const iconMap = { error: '⚠️', warning: '⚠️', info: 'ℹ️' };
                return (
                  <div key={i} className={`px-4 py-2 rounded-lg border text-sm ${styleMap[w.severity]}`}>
                    <span className="mr-2">{iconMap[w.severity]}</span>
                    {w.message}
                  </div>
                );
              })}
          ...
          {/* Phase cards with summary data */}
          <div className="space-y-3">
            {sortedPhases.map(phase => {
              const status = PHASE_STATUSES[phase.status] || PHASE_STATUSES.working;
              const summary = summarizePhaseFromData(phase, accounts, incomeStreams, recurringExpenses, personalInfo);
              const endAge = phase.endAge !== null && phase.endAge !== undefined ? phase.endAge : personalInfo.myLifeExpectancy;
              return (
                <div key={phase.id} className={cardStyle}>
                  <div className="flex items-start gap-4">
                    <div 
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        <span className="text-xs px-2 py-0.5 rounded-full border" style={{ borderColor: status.color + '60', color: status.color, backgroundColor: status.color + '20' }}>
                          {status.icon} {status.label}
                        </span>
                        <span className="text-sm text-slate-400">
                          Age {phase.startAge}–{endAge} ({summary.durationYears} years)
                        </span>
                      </div>
                      
                      {phase.notes && (
                        <p className="text-sm text-slate-400 mb-3">{phase.notes}</p>
                      )}
                      
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 text-sm">
                        <div>
                          <div className="text-xs text-slate-500">Earned Income</div>
                          <div className="text-base font-semibold text-emerald-300">
                            {summary.earnedIncomeAtStart > 0 ? formatCurrency(summary.earnedIncomeAtStart) + '/yr' : '—'}
                          </div>
                        </div>
                        <div>
                          <div className="text-xs text-slate-500">Other Income</div>
                          <div className="text-base font-semibold text-blue-300">
                            {summary.otherIncomeAtStart > 0 ? formatCurrency(summary.otherIncomeAtStart) + '/yr' : '—'}
                          </div>
                        </div>
                        <div>
                          <div className="text-xs text-slate-500">Contributions</div>
                          <div className="text-base font-semibold text-amber-300">
                            {summary.contributionsAtStart > 0 ? formatCurrency(summary.contributionsAtStart) + '/yr' : '—'}
                          </div>
                        </div>
                        <div>
                          <div className="text-xs text-slate-500">Recurring Expenses</div>
                          <div className="text-base font-semibold text-purple-300">
                            {summary.recurringAtStart > 0 ? formatCurrency(summary.recurringAtStart) + '/yr' : '—'}
                          </div>
                        </div>
                      </div>
                      <div className="text-xs text-slate-500 mt-2">
                        Values shown at age {phase.startAge} (phase start). Edit specific income streams in the Income tab, contributions in Accounts, expenses in Personal Info.
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 flex-shrink-0">
                      ...
                      <button
                        onClick={() => {
                          if (window.confirm(`Delete phase "${phase.name}"?`)) deletePhase(phase.id);
                        }}
                        className="text-xs px-2 py-1 rounded border bg-red-500/10 text-red-300 border-red-500/30 hover:bg-red-500/20 transition-all"
                      >
                        🗑️ Delete
          ...
          <div className="flex justify-end">
            <button
              onClick={() => { setEditingPhase(null); setShowAddModal(true); }}
              className={buttonPrimary}
            >
              + Add Phase
            </button>
      ...
      {showAddModal && (
        <PhaseModal
          phase={editingPhase}
          onSave={savePhase}
          onCancel={() => { setShowAddModal(false); setEditingPhase(null); }}
          personalInfo={personalInfo}
        />
      )}
    </div>
...
function PhaseModal({ phase, onSave, onCancel, personalInfo }) {
  const [formData, setFormData] = useState(phase || {
    name: '',
    status: 'working',
    startAge: personalInfo.myAge,
    endAge: personalInfo.myRetirementAge - 1,
    notes: '',
  });
  ...
  const handleSave = () => {
    ...
    if (formData.startAge >= formData.endAge && formData.endAge !== null) {
      alert('Phase end age must be greater than start age');
      return;
    }
    onSave(formData);
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className={labelStyle}>Phase name</label>
            <input
              type="text"
              value={formData.name}
              onChange={e => setFormData({...formData, name: e.target.value})}
              className={inputStyle}
              placeholder="e.g., Consulting Years, Sabbatical, Working Years"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelStyle}>Start age</label>
              <input
                type="number"
            ...
            <div>
              <label className={labelStyle}>End age</label>
              <input
                type="number"
                value={formData.endAge === null || formData.endAge === undefined ? '' : formData.endAge}
                onChange={e => setFormData({...formData, endAge: e.target.value === '' ? null : Number(e.target.value)})}
                className={inputStyle}
                min={18} max={120}
                placeholder="Leave blank for end of life"
              />
              <p className="text-xs text-slate-500 mt-1">Blank = until life expectancy ({personalInfo.myLifeExpectancy})</p>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className={inputStyle}
              rows={2}
              placeholder="What characterizes this phase?"
            />
          </div>
          
          <div className="p-3 bg-slate-900/50 rounded-lg border border-slate-700">
            <p className="text-xs text-slate-400">
              <strong className="text-slate-300">Note:</strong> Phases are descriptive — they don't directly change your projections. To actually model phase-specific income, contributions, or expenses, edit those items in the <strong>Income</strong>, <strong>Accounts</strong>, and <strong>Personal Info</strong> tabs (their start/end ages should align with your phase boundaries).
            </p>
          </div>
        </div>
...
// ============================================
// CashFlowTab — Contribution priority optimizer
// ============================================
function CashFlowTab({ priorityRules, setPriorityRules, accounts, setAccounts, incomeStreams, recurringExpenses, personalInfo, projections }) {
  const [showAddRuleModal, setShowAddRuleModal] = useState(false);
  
  // Compute current allocation
  const allocation = useMemo(() => {
    return computeCashFlowAllocation(priorityRules, accounts, incomeStreams, recurringExpenses, personalInfo, projections);
  }, [priorityRules, accounts, incomeStreams, recurringExpenses, personalInfo, projections]);
  
  // Move a rule up or down (locked rules can't move)
  const moveRule = (id, direction) => {
    setPriorityRules(prev => {
      ...
      // Can't swap with a locked rule
      if (prev[newIdx].locked || prev[idx].locked) return prev;
      const arr = [...prev];
  ...
  const updateRuleAmount = (id, amount) => {
    setPriorityRules(prev => prev.map(r => 
      r.id === id ? { ...r, amount: Number(amount) } : r
    ));
  };
  
  const updateRulePercent = (id, percent) => {
    setPriorityRules(prev => prev.map(r => 
      r.id === id ? { ...r, percentOfSalary: Number(percent) } : r
    ));
  };
  
  const deleteRule = (id) => {
    setPriorityRules(prev => prev.filter(r => r.id !== id || r.locked));
  };
  
  const resetToDefaults = () => {
    if (window.confirm('Reset priority rules to defaults? Your custom ordering and amounts will be lost.')) {
      setPriorityRules(DEFAULT_PRIORITY_RULES);
    }
  };
  
  // Apply recommended contributions to actual accounts.
  // Maps priority types to account types and updates the contribution field.
  const applyToAccounts = () => {
    if (!window.confirm(
      'This will update your account contribution values to match the optimizer recommendations. ' +
      'You can manually edit them afterwards. Continue?'
    )) return;
    
    const updates = new Map();  // accountId -> new contribution
    
    allocation.allocations.forEach(a => {
      if (a.allocated <= 0) return;
      
      // Map priority types to account types
      const typeMap = {
        employer_match: ['401k', '403b', 'tsp'],
        roth_ira: ['roth_ira'],
        retirement_max: ['401k', '403b', 'tsp'],
        hsa: ['hsa'],
        brokerage_savings: ['brokerage'],
      };
      
      const accountTypes = typeMap[a.type];
      if (!accountTypes) return;
      
      // Find first matching account
      const target = accounts.find(acc => accountTypes.includes(acc.type) && acc.owner === 'me');
      if (!target) return;
      
      // For employer_match + retirement_max, sum them onto the same 401k
      const existing = updates.get(target.id) || 0;
      updates.set(target.id, existing + a.allocated);
    });
    
    setAccounts(prev => prev.map(acc => {
      if (updates.has(acc.id)) {
        return { ...acc, contribution: Math.round(updates.get(acc.id)) };
      }
      return acc;
    }));
  };
  ...
  // Status colors
  const statusColors = {
    allocated: 'text-emerald-300 bg-emerald-500/10 border-emerald-500/30',
    partial:   'text-amber-300 bg-amber-500/10 border-amber-500/30',
    unfunded:  'text-red-300 bg-red-500/10 border-red-500/30',
    disabled:  'text-slate-500 bg-slate-800/50 border-slate-700',
    na:        'text-slate-400 bg-slate-800/50 border-slate-700',
  };
  const statusLabels = {
    allocated: '✓ Funded',
    partial:   '⚠️ Partial',
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xl font-semibold text-slate-100 mb-2">Cash-Flow Priority Optimizer</h3>
        <p className="text-slate-400 text-sm">
          Define your priorities for surplus dollars. The optimizer takes your current income and essential expenses,
          calculates surplus, then walks down this list to recommend how to allocate it. Drag arrows to reorder. 
          Click "Apply to Accounts" when you're satisfied — it updates your account contribution values.
        </p>
      </div>
      
      {/* Income breakdown card */}
      <div className={cardStyle}>
        <h4 className="text-base font-semibold text-slate-100 mb-3">Cash-Flow Snapshot (current year, age {personalInfo.myAge})</h4>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
          <div>
            <div className="text-xs text-slate-500">Gross Income</div>
            <div className="text-lg font-semibold text-emerald-300">{formatCurrency(allocation.totalIncome)}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">– Taxes</div>
            <div className="text-lg font-semibold text-red-300">{formatCurrency(allocation.totalTax)}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">– Essential expenses</div>
            <div className="text-lg font-semibold text-orange-300">{formatCurrency(allocation.totalEssential)}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500">= Surplus</div>
            <div className={`text-lg font-semibold ${allocation.surplus < 0 ? 'text-red-400' : 'text-amber-300'}`}>
              {formatCurrency(allocation.surplus)}
            </div>
          </div>
          <div>
            <div className="text-xs text-slate-500">Unallocated</div>
            <div className="text-lg font-semibold text-slate-300">{formatCurrency(allocation.unallocated)}</div>
          </div>
        </div>
        {allocation.surplus < 0 && (
          <div className="mt-3 px-3 py-2 bg-red-900/20 border border-red-500/40 rounded text-sm text-red-300">
            ⚠️ Negative surplus: your essential expenses + tax exceed gross income. Review your inputs.
          </div>
        )}
      </div>
      
      {/* Priority rules — ordered list */}
      <div className={cardStyle}>
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-base font-semibold text-slate-100">Priority Order</h4>
          <div className="flex gap-2">
            <button onClick={resetToDefaults} className={`text-xs ${buttonSecondary}`}>↺ Reset to defaults</button>
            <button onClick={applyToAccounts} className={`text-xs ${buttonPrimary}`}>📋 Apply to Accounts</button>
          </div>
        </div>
        ...
        <div className="space-y-2">
          {allocation.allocations.map((a, i) => {
            const isFirst = i === 0;
            const isLast = i === allocation.allocations.length - 1;
            return (
              <div
                key={a.id}
                className={`flex items-center gap-3 p-3 rounded-lg border ${
                  a.enabled ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-800/20 border-slate-700/50 opacity-60'
                }`}
              >
                {/* Order number */}
                ...
                <div className="flex flex-col gap-0.5 flex-shrink-0">
                  <button
                    onClick={() => moveRule(a.id, -1)}
                    disabled={isFirst || a.locked}
                    className="text-xs w-6 h-5 rounded bg-slate-700 hover:bg-slate-600 disabled:opacity-30 disabled:cursor-not-allowed text-slate-300"
                    title="Move up"
                  >
                    ▲
                  </button>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-slate-100">{a.def.label}</span>
                    {a.locked && <span className="text-xs text-slate-500">🔒</span>}
                  </div>
                  <div className="text-xs text-slate-400 mt-0.5">{a.def.description}</div>
                </div>
                
                {/* Amount input (varies by type) */}
                <div className="flex-shrink-0 w-32">
                  {a.type === 'employer_match' ? (
                    <div>
                      <input
                        type="number"
                        step="0.5"
                        min={0}
                        max={50}
                        value={a.percentOfSalary || 6}
                        onChange={e => updateRulePercent(a.id, e.target.value)}
                        disabled={!a.enabled}
                        className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-sm text-slate-100 disabled:opacity-50"
                      />
                      <div className="text-xs text-slate-500 mt-0.5">% of salary</div>
                    </div>
                  ) : a.type === 'brokerage_savings' || a.type === 'high_int_debt' || a.type === 'med_int_debt' || a.type === 'low_int_debt' || a.type === 'debt_min' ? (
                    <div className="text-xs text-slate-500 italic text-center pt-2">
                      {a.def.takesAll ? 'Takes remaining' : 'Auto'}
                    </div>
                  ) : (
                    <div>
                      <input
                        ...
                        step="500"
                        min={0}
                        value={a.amount !== undefined ? a.amount : (a.def.defaultAmount || 0)}
                        onChange={e => updateRuleAmount(a.id, e.target.value)}
                        disabled={!a.enabled}
                        className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-sm text-slate-100 disabled:opacity-50"
                      />
                ...
                <div className="flex-shrink-0 w-32 text-right">
                  ...
                  <span className={`inline-block text-xs px-2 py-0.5 rounded border ${statusColors[a.status]}`}>
                ...
                <div className="flex-shrink-0">
                  <button
                    ...
                    className={`text-xs px-2 py-1 rounded border ${
                      a.enabled ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-slate-700 border-slate-600 text-slate-400'
                    } disabled:opacity-50`}
                  >
                    {a.enabled ? '✓ On' : '○ Off'}
                  </button>
        ...
        {allocation.unallocated > 0 && (
          <div className="mt-4 px-3 py-2 bg-amber-900/20 border border-amber-500/40 rounded text-sm text-amber-300">
            💰 <strong>{formatCurrency(allocation.unallocated)}</strong> unallocated. The last enabled rule is "{allocation.allocations.filter(a => a.enabled).slice(-1)[0]?.def.label || 'unknown'}" — consider enabling more priorities below it, or it just stays in your checking account.
          </div>
        )}
      </div>
      ...
      {/* Help/notes card */}
      <div className={cardStyle}>
        <h4 className="text-base font-semibold text-slate-100 mb-2">How this works</h4>
        <ul className="space-y-1.5 text-sm text-slate-400">
          <li>• <strong className="text-slate-300">Surplus = Gross Income − Tax − Essential Expenses</strong>. Essentials = your "current expenses" before retirement, "desired retirement income" after, plus active recurring expenses.</li>
          <li>• Each rule pulls from surplus in priority order. When surplus runs out, lower-priority rules show as "Unfunded".</li>
          <li>• <strong className="text-slate-300">Brokerage / taxable</strong> is special: it absorbs whatever surplus is left after the priorities above it. Put it last (or near-last) for "save the rest" behavior.</li>
          <li>• The <strong className="text-slate-300">Apply to Accounts</strong> button writes the recommended dollar amounts to your account.contribution fields. After applying, your projections will reflect the new contributions.</li>
          <li>• Debt-related rules require debt accounts (we don't model debts as accounts yet — debt rules are placeholders for now).</li>
        </ul>
      </div>
    </div>
  );
...
function RetirementPlanner() {
  ...
  const [milestones, setMilestones] = useState(() => {
    return savedData?.milestones || DEFAULT_MILESTONES;
  });
  const [phases, setPhases] = useState(() => {
    return savedData?.phases || DEFAULT_PHASES;
  });
  const [priorityRules, setPriorityRules] = useState(() => {
    return savedData?.priorityRules || DEFAULT_PRIORITY_RULES;
  });
  ...
  useEffect(() => {
    const saveTimer = setTimeout(() => {
      const data = { personalInfo, accounts, incomeStreams, assets, oneTimeEvents, recurringExpenses, milestones, phases, priorityRules, dashboardVisibility, scenarios, lastSaved: new Date().toISOString() };
      saveToStorage(data);
      setSaveStatus('Saved');
    }, SAVE_DEBOUNCE_MS);
  ...
  }, [personalInfo, accounts, incomeStreams, assets, oneTimeEvents, recurringExpenses, milestones, phases, priorityRules, dashboardVisibility, scenarios]);
  ...
  const handleExport = () => {
    const data = {
      ...
      version: '2.0'  // v34 - Cash-flow priority engine added
    };
  ...
  const handleImport = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (data.priorityRules) setPriorityRules(data.priorityRules);
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="max-w-7xl mx-auto">
            {activeTab === 'dashboard' && <DashboardTab accounts={accounts} assets={assets} computeProjections={computeProjections} dashboardVisibility={dashboardVisibility} incomeStreams={incomeStreams} milestones={milestones} oneTimeEvents={oneTimeEvents} personalInfo={personalInfo} phases={phases} projections={projections} recurringExpenses={recurringExpenses} setDashboardVisibility={setDashboardVisibility} setShowDashboardSettings={setShowDashboardSettings} showDashboardSettings={showDashboardSettings} />}
            {activeTab === 'milestones' && <MilestonesTab milestones={milestones} setMilestones={setMilestones} projections={projections} personalInfo={personalInfo} />}
            {activeTab === 'phases' && <PhasesTab phases={phases} setPhases={setPhases} accounts={accounts} incomeStreams={incomeStreams} recurringExpenses={recurringExpenses} personalInfo={personalInfo} />}
            {activeTab === 'cashflow' && <CashFlowTab priorityRules={priorityRules} setPriorityRules={setPriorityRules} accounts={accounts} setAccounts={setAccounts} incomeStreams={incomeStreams} recurringExpenses={recurringExpenses} personalInfo={personalInfo} projections={projections} />}
