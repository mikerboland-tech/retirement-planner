...
section('P113 — the Content-Security-Policy: current, strict, and complete');

{
  // The policy is a <meta> generated by tools/build.cjs. Regenerated here the
  // way the browser reads it — the SHA-256 of each inline script's exact text
  // — and compared, so a stale build fails the suite rather than shipping a
  // page whose own scripts the browser refuses to run.
  const fs = require('fs'), pathMod = require('path'), crypto = require('crypto');
  const ROOT = pathMod.resolve(__dirname, '..');
  const scan = (html) => {
    const out = [];
    ...
    while ((m = re.exec(stripped))) {
      const attrs = m[1] || '';
      if (/\bsrc\s*=/.test(attrs) || /text\/babel/.test(attrs) || !m[2].trim()) continue;
      out.push("'sha256-" + crypto.createHash('sha256').update(m[2], 'utf8').digest('base64') + "'");
    }
    return out;
  };
  for (const file of ['index.html', 'mobile.html']) {
    const html = fs.readFileSync(pathMod.join(ROOT, file), 'utf8');
    const meta = /<meta http-equiv="Content-Security-Policy" content="([^"]+)">/.exec(html);
    ok(meta, `${file} carries a Content-Security-Policy meta`);
    const policy = meta ? meta[1] : '';
    // Located on the comment-stripped page: the explanatory comment above the
    // meta mentions "<script>" in prose.
    const bare = html.replace(/<!--[\s\S]*?-->/g, '');
    const headStart = bare.indexOf('<head>'), metaAt = bare.indexOf('<meta http-equiv="Content-Security-Policy"');
    const firstScript = bare.indexOf('<script', headStart);
    ok(metaAt > headStart && metaAt < firstScript, `${file}: the policy precedes every script, so it governs them all`);
    const directive = (name) => { const m = new RegExp('(?:^|;\\s*)' + name + ' ([^;]+)').exec(policy); return m ? m[1] : ''; };
    const scriptSrc = directive('script-src');
    const hashes = scan(html);
    gt(hashes.length, 3, `${file} has inline scripts to allow (${hashes.length})`);
    hashes.forEach((h, i) => ok(scriptSrc.includes(h), `${file}: inline script ${i + 1} is allowed by hash`));
    eq((scriptSrc.match(/'sha256-/g) || []).length, hashes.length, `${file}: and no stale hash lingers`);
    ok(!/unsafe-eval|unsafe-inline/.test(scriptSrc), `${file}: script-src allows neither eval nor inline`);
    ok(/'self' blob:/.test(scriptSrc), `${file}: app code runs from a Blob URL`);
    eq(directive('object-src'), "'none'", `${file}: no plugins`);
    eq(directive('base-uri'), "'self'", `${file}: no base hijack`);
    ok(directive('worker-src').includes("'self'"), `${file}: the worker may load`);
    const connect = directive('connect-src');
    eq(connect, "'self' https://api.anthropic.com", `${file}: the assistant's host is the only remote the page may talk to`);
    ok(!/\(0,\s*eval\)|new Function\(/.test(html.replace(/<!--[\s\S]*?-->/g, '').replace(/\/\/[^\n]*/g, '')),
      `${file} no longer evaluates code through eval or new Function`);
  }
  // The assistant's URL in the app is the host the policy names.
  const jsx = fs.readFileSync(pathMod.join(ROOT, 'retirement-planner.jsx'), 'utf8');
  const url = /const AI_API_URL = ['"]([^'"]+)['"]/.exec(jsx);
  ok(url && url[1].startsWith('https://api.anthropic.com/'), 'the assistant talks to api.anthropic.com, which connect-src allows');
  // Nothing else in the app fetches a remote host.
  const remoteFetches = (jsx.match(/fetch\((['"`])https?:\/\/[^'"`]+/g) || []).filter(f => !f.includes('api.anthropic.com'));
